...
async function ensureDojoId(dojoName: string): Promise<string | null> {
  const { data: existing } = await supabaseAdmin.from("dojos").select("id").eq("name", dojoName).maybeSingle();
  if (existing) return existing.id;

  const { data: created } = await supabaseAdmin.from("dojos").insert({ name: dojoName }).select("id").single();
...
function buildFighterFields(entry: Entry, dojoId: string) {

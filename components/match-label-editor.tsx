...
export function MatchLabelEditor({
  ...
  function handleNumberClick(matchId: string) {
    setOrder((prev) => (prev.includes(matchId) ? prev.filter((id) => id !== matchId) : [...prev, matchId]));
  }
  ...
  async function handleSwapWithNext(round: number, matchId: string) {
    ...
    const tournament = tournaments.find((t) => t.matches.some((m) => m.id === matchId));
    if (!tournament) return;
    const roundMatches = tournament.matches.filter((m) => m.round === round).sort((a, b) => a.position - b.position);
  ...
  const totalCount = tournaments.reduce(
  ...
  const allLabeledInDb = useMemo(() => {
    ...
    const labeledCount = tournaments.reduce(
      (s, t) =>
    ...
    <div className="border border-gray-700 rounded-xl overflow-hidden">
      ...
      <div className="bg-gray-900 p-4">
        <p className="text-xs text-gray-400 mb-3">
          ...
          Aコート第1試合）。 番号をつけたカードをもう一度タップすると解除します。

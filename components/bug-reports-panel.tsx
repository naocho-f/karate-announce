...
function ReportEditSection({
  ...
  editFixedVersion: string;
  saving: boolean;
  setEditStatus: (v: string) => void;
  setEditResolution: (v: string) => void;
  setEditFixedVersion: (v: string) => void;
  onSave: (id: string) => void;
}) {
  return (
    <div className="space-y-2 bg-gray-900 rounded p-2">
      <div className="flex items-center gap-2">
        <label htmlFor="bug-status" className="text-xs text-gray-400">
          ステータス
        </label>
        <select
          id="bug-status"
      ...
      <div>
        <label htmlFor="bug-resolution" className="text-xs text-gray-400">
          対応内容（原因と修正内容）
        </label>
        <textarea
          id="bug-resolution"
      ...
      <div className="flex items-center gap-2">
        <label htmlFor="bug-fixed-version" className="text-xs text-gray-400">
          修正バージョン
        </label>
        <input
          id="bug-fixed-version"
...
function BugReportsHeader({
  ...
  hasOpen: boolean;
  onSetFilter: (f: "all" | "open" | "in_progress" | "resolved" | "wontfix") => void;
}) {
  return (
      ...
      <div className="flex gap-1 ml-auto">
        {FILTER_BUTTONS.map((f) => (
          <button
            ...
            className={`px-2 py-0.5 rounded-full text-xs transition ${filter === f.key ? "bg-blue-600 text-white" : "bg-gray-700 text-gray-400 hover:bg-gray-600"}`}
          >
            {f.label}
          </button>
        ))}
  ...
  return (
    <div className="space-y-3">
      <BugReportsHeader
        ...
        hasOpen={reports.some((r) => r.status === "open")}
        ...
        >
          <div className="px-3 pb-3 space-y-3 border-t border-gray-700">
            ...
            <ReportEditSection
              ...
              editFixedVersion={editFixedVersion}
              saving={saving}
              setEditStatus={setEditStatus}
              setEditResolution={setEditResolution}
              setEditFixedVersion={setEditFixedVersion}
              onSave={(id) => void saveReport(id)}
            />
          </div>
        </ReportRow>
      ))}

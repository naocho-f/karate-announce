...
async function callMatchApi(matchId: string, payload: Record<string, unknown>, offlineMode: string, tabId: string): Promise<MatchApiResult> {
  try {
    const res = await resilientFetch(`/api/court/matches/${matchId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }, { maxRetries: 3, timeout: 5000, offlineMode: offlineMode === "offline" });
    return res.ok ? "ok" : "failed";
  } catch {
    await enqueue({ action: payload.action as CourtAction, endpoint: `/api/court/matches/${matchId}`, method: "PATCH", payload, createdAt: new Date().toISOString(), tabId });
    return "queued";
  }
}
...
type CourtPanelData = {
  tournaments: Tournament[]; matchesMap: Record<string, Match[]>;
  fighters: Record<string, Fighter>; withdrawnFighterIds: Set<string>;
...
function useCourtPanelData(courtNum: string): CourtPanelData & { load: () => Promise<void> } {
  ...
  const load = useCallback(async () => {
    const { data: tourns } = await supabase.from("tournaments").select("*").eq("court", courtNum).neq("status", "finished").order("sort_order").order("created_at");
    if (!tourns?.length) { setTournaments([]); setMatchesMap({}); return; }
    ...
    (allMatches ?? []).forEach((m) => { if (m.fighter1_id) allFighterIds.add(m.fighter1_id); if (m.fighter2_id) allFighterIds.add(m.fighter2_id); });
    ...
    if (allFighterIds.size > 0 && eventId) {
      const { data: e } = await supabase.from("entries").select("id, fighter_id, is_withdrawn").eq("event_id", eventId).in("fighter_id", [...allFighterIds]);
      allEntries = e ?? [];
    }
    ...
    allEntries.forEach((e) => { if (e.fighter_id) { entryMap[e.fighter_id] = e.id; if (e.is_withdrawn) withdrawn.add(e.fighter_id); } });
  ...
  useEffect(() => {
    ...
    const handleVisibility = () => { if (document.visibilityState === "visible") doLoad(); };
    document.addEventListener("visibilitychange", handleVisibility);
    return () => { cancelled = true; clearInterval(timer); document.removeEventListener("visibilitychange", handleVisibility); };
  }, [load]);
...
function doStartAnnounceIndex(
  match: Match, rounds: number, tournamentId: string,
  fighters: Record<string, Fighter>, tournaments: Tournament[],
  ...
  void announceMatchStart(
    ...
    roundName(match.round, rounds), fighterFullReading(f1), f1.affiliation_reading ?? f1.dojo?.name_reading,
...
function useCourtIndexActions(
  ...
  const doWinAnn = async (mId: string, wId: string) => {
    ...
    await announceWinner(fighterFullName(w), fighterAff(w), fighterFullReading(w), w.affiliation_reading ?? w.dojo?.name_reading, announceTemplates);
  };
  const startMatch = async (tId: string, mId: string) => {
    ...
    if (r !== "ok") { endP(mId); return; }
    await load(); endP(mId);
    if (!mutedMatchIds.has(mId)) doStartAnnounceIndex(match, rounds, tId, fighters, tournaments, announceTemplates, rulesReadingMap);
  };
  const setWinner = async (tId: string, mId: string, wId: string) => {
    const { match, rounds } = getCtx(tId, mId);
    if (!match || !fighters[wId]) return;
    startP(mId); addPendingWinner(mId);
    const r = await callMatchApi(mId, { action: "set_winner", winnerId: wId, tournamentId: tId, round: match.round, rounds, position: match.position }, offlineMode, "court-index");
    if (r !== "ok") { endP(mId); removePendingWinner(mId); return; }
    await load(); removePendingWinner(mId); endP(mId); await doWinAnn(mId, wId);
  };
  const correctWinner = async (tId: string, mId: string, wId: string) => {
    const { match, rounds } = getCtx(tId, mId);
    if (!match || !fighters[wId]) return;
    startP(mId);
    const r = await callMatchApi(mId, { action: "correct_winner", winnerId: wId, tournamentId: tId, round: match.round, rounds, position: match.position }, offlineMode, "court-index");
    if (r !== "ok") { endP(mId); return; }
    await load(); endP(mId); await doWinAnn(mId, wId);
  };
  const toggleWithdrawal = async (mId: string, eId: string, w: boolean) => {
    startP(mId);
    try { await resilientFetch(`/api/court/entries/${eId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ is_withdrawn: w }) }, { maxRetries: 3, timeout: 5000, offlineMode: offlineMode === "offline" }); } catch { endP(mId); return; }
    await load(); endP(mId);
  };
  const toggleMute = (mId: string) => { setMutedMatchIds((p) => { const n = new Set(p); if (n.has(mId)) n.delete(mId); else n.add(mId); localStorage.setItem("muted_match_ids", JSON.stringify([...n])); return n; }); };
  const reannounceStart = async (tId: string, mId: string) => { const { match, rounds } = getCtx(tId, mId); if (match) doStartAnnounceIndex(match, rounds, tId, fighters, tournaments, announceTemplates, rulesReadingMap); };
  const reannounceWinner = async (tId: string, mId: string) => { const { match } = getCtx(tId, mId); if (match?.winner_id) await doWinAnn(mId, match.winner_id); };
  const swapWithNext = async (tId: string, round: number, mId: string) => {
    ...
    const next = rm[idx + 1];
    startP(mId); startP(next.id);
    try { await resilientFetch(`/api/court/matches/${mId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "swap_with", otherMatchId: next.id }) }, { maxRetries: 3, timeout: 5000, offlineMode: offlineMode === "offline" }); }
    catch { await enqueue({ action: "swap_with", endpoint: `/api/court/matches/${mId}`, method: "PATCH", payload: { action: "swap_with", otherMatchId: next.id }, createdAt: new Date().toISOString(), tabId: "court-index" }); endP(mId); endP(next.id); return; }
    await load(); endP(mId); endP(next.id);
  };
  return { processingMatchIds, mutedMatchIds, startMatch, setWinner, correctWinner, toggleWithdrawal, toggleMute, reannounceStart, reannounceWinner, swapWithNext };
}

function CourtPanel({
  courtNum, courtDisplayName, announceTemplates, rulesReadingMap,
}: {
  courtNum: string; courtDisplayName: string; announceTemplates: AnnounceTemplates; rulesReadingMap: Record<string, string>;
}) {
      ...
      {tournaments.length === 0 ? (
        ...
        <div className="space-y-6">
          {tournaments.map((tournament) => (
            <CourtTournamentCard
              key={tournament.id} tournament={tournament} matches={matchesMap[tournament.id] ?? []}
              nameMap={nameMap} affiliationMap={affiliationMap}
              withdrawnIds={withdrawnFighterIds} fighterEntryMap={fighterEntryMap}
              processingMatchIds={actions.processingMatchIds} mutedMatchIds={actions.mutedMatchIds}
...
function CourtTournamentCard(props: {
  ...
  onStartMatch: (mId: string) => void; onSetWinner: (mId: string, fId: string) => void;
  onCorrectWinner: (mId: string, fId: string) => void;
  onReannounceStart: (mId: string) => void; onReannounceWinner: (mId: string) => void;
  onToggleWithdrawal: (mId: string, fId: string, eId: string, w: boolean) => void;
  onSwapWithNext: (round: number, mId: string) => void; onToggleMute: (mId: string) => void;
}) {
      ...
      <div className="bg-gray-800 rounded-xl p-4">
        {matches.length === 0 ? (
          ...
          <BracketView
            ...
            withdrawnIds={props.withdrawnIds} fighterEntryMap={props.fighterEntryMap}
            processingMatchIds={props.processingMatchIds} mutedMatchIds={props.mutedMatchIds}
            onMatchClick={props.onStartMatch} onSetWinner={props.onSetWinner}
            onCorrectWinner={props.onCorrectWinner} onReannounceStart={props.onReannounceStart}
            onReannounceWinner={props.onReannounceWinner}
            onWithdrawnToggle={props.onToggleWithdrawal}
            onSwapWithNext={props.onSwapWithNext} onToggleMute={props.onToggleMute}
          />
        )}
      </div>
    </div>

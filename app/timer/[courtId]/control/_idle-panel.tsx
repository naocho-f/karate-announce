...
export default function IdlePanel({
  ...
  return (
    <section className="space-y-3">
      ...
      {presets.length > 0 && (
        <div>
          <label htmlFor="idle-preset-select" className="text-xs text-gray-400">
            ルール
          </label>
          <select
            id="idle-preset-select"
    ...
    <div className="space-y-2" ref={matchListTopRef}>
      ...
      {matchCandidates.map((c) => (
        <MatchCard
          key={c.match.id}
          candidate={c}
          isFirstReady={c.match.status === "ready" && c.match.id === firstReadyId}
          onRef={(el) => {
...
const HEADER_BG: Record<MatchStatus, string> = {
  done: "bg-gray-800/50",
  waiting: "bg-gray-800/30",
  ongoing: "bg-yellow-900/40",
  firstReady: "bg-blue-900/30",
  ready: "bg-gray-800/30",
};
const LABEL_COLOR: Record<MatchStatus, string> = {
  done: "text-gray-600",
  waiting: "text-gray-500",
  ongoing: "text-yellow-300",
  firstReady: "text-blue-300",
  ready: "text-gray-300",
};
  ...
  return (
    ...
    >
      ...
      <div className="px-3 py-2">
        <FighterRow
          fighter={c.fighter1}
          sideColor={dimmed ? "bg-gray-700" : "bg-red-600"}
          nameColor={dimmed ? "text-gray-600" : "text-red-400"}
          showAff={!dimmed}
        />
        <div className="text-center text-gray-600 text-xs my-0.5">vs</div>
        <FighterRow
          fighter={c.fighter2}
          sideColor={dimmed ? "bg-gray-700" : "bg-gray-300"}
          nameColor={dimmed ? "text-gray-600" : "text-gray-200"}
          showAff={!dimmed}
        />
        <div className="flex items-center gap-2 mt-1.5 pt-1.5 border-t border-gray-800/60">
          {rulesLabel && (
            <span
              className={`text-[10px] px-1.5 py-0.5 rounded ${ms === "done" ? "bg-gray-800 text-gray-600" : "bg-gray-800 text-gray-400"}`}
            >
              {rulesLabel}
            </span>
          )}

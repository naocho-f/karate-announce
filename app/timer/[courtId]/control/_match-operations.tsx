  ...
  return (
    <>
      ...
      {(phase === "running" || phase === "paused" || phase === "time_up") && (
        <ScoringPanel state={props.state} preset={props.p} swapSides={props.swapSides}
          onAddPoint={(side) => props.onUpdate((s) => addPoint(s, side))} onAddWazaari={(side) => props.onUpdate((s) => addWazaari(s, side))}
          onAddFoul={(side) => props.onUpdate((s) => addFoul(s, side))} onIpponConfirm={(side) => props.onSetIpponConfirmSide(side)} />
      )}
      ...
      {(phase === "time_up" || phase === "finished") && (
        <ResultPanel state={props.state} preset={props.p} swapSides={props.swapSides}
          selectingResultFor={props.selectingResultFor} writingBack={props.writingBack}
          onSelectingResultFor={props.onSetSelectingResultFor}
          onFinishManual={(side, method) => props.onUpdate((s) => finishManual(s, side, method))}
          onWriteBack={props.onWriteBack} onCancelResult={() => props.onUpdate(cancelResult)} onResetToIdle={props.onResetToIdle} />
      )}
...
function BackToListButton({ props: { phase, showAnnounceSelection, onSetShowAnnounceSelection, onUpdate, onLoadTournamentData } }: { props: MatchOperationsProps }) {
  if (!(showAnnounceSelection || phase === "ready" || phase === "running" || phase === "paused")) return null;
  return <button onClick={() => { onSetShowAnnounceSelection(false); onUpdate(resetToIdle); onLoadTournamentData(); }} className="text-sm text-gray-500 hover:text-gray-300 transition">← 試合一覧に戻る</button>;
}

function AnnounceSelection({ props: { isMuted, isPlaying, onSetShowAnnounceSelection, onAnnounceStart } }: { props: MatchOperationsProps }) {
  return (
    <section className="space-y-3">
      <button onClick={() => { onSetShowAnnounceSelection(false); onAnnounceStart(); }} disabled={isMuted || isPlaying} className="w-full py-4 rounded-lg bg-blue-700 hover:bg-blue-600 text-white font-bold text-lg transition disabled:opacity-40 disabled:cursor-not-allowed">{isPlaying ? "再生中..." : "🔊 開始アナウンスを再生"}</button>
      <button onClick={() => onSetShowAnnounceSelection(false)} className="w-full py-4 rounded-lg bg-gray-700 hover:bg-gray-600 text-gray-300 font-bold text-lg transition">アナウンスなしで試合準備画面へ</button>
  ...
  return (
    <section>
      ...
      <div className="flex gap-2">
        {(phase === "ready" || phase === "extension") && <button onClick={() => onUpdate(startTimer)} className="flex-1 py-6 rounded-lg bg-green-700 hover:bg-green-600 text-white font-bold text-xl transition">▶ 開始 [Space]</button>}
...
function WinnerAnnounce({ props: { isMuted, isPlaying, onAnnounceWinner } }: { props: MatchOperationsProps }) {
  return (
    <section>
      ...
      <button onClick={onAnnounceWinner} disabled={isMuted || isPlaying} className="w-full py-2 rounded-lg bg-purple-700 hover:bg-purple-600 text-white font-bold text-sm transition disabled:opacity-40 disabled:cursor-not-allowed">{isPlaying ? "再生中..." : "勝利アナウンス"}</button>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 mx-4 max-w-sm w-full space-y-4">
        <p className="text-center text-lg font-bold text-white">{sideLabel}（{sideName}）の一本を記録しますか？</p>
        <div className="grid grid-cols-2 gap-3">
          <button onClick={() => { onUpdate((s) => addIppon(s, ipponConfirmSide)); onSetIpponConfirmSide(null); }} className="py-4 rounded-lg bg-red-700 hover:bg-red-600 text-white font-bold text-lg transition">一本を記録</button>
          <button onClick={() => onSetIpponConfirmSide(null)} className="py-4 rounded-lg bg-gray-700 hover:bg-gray-600 text-gray-300 font-bold text-lg transition">キャンセル</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-5 gap-2">
        <button onClick={() => void playBuzzer(p?.buzzer_sound ?? "mid-square-single", p?.buzzer_duration ?? 1.5, p?.buzzer_repeat ?? 1).then((r) => { if (r === "fallback") onSetBuzzerWarning(true); })}
          className={`py-2 rounded bg-gray-700 hover:bg-gray-600 text-gray-300 text-sm transition ${canAdjust ? "" : "col-span-5"}`}>ブザー [B]</button>
        {canAdjust && (
          <>
            <button onClick={() => onUpdate((s) => adjustTime(s, -10000))} className="py-2 rounded bg-gray-700 hover:bg-gray-600 text-gray-300 text-sm transition">-10秒 [←]</button>
            <button onClick={() => onUpdate((s) => adjustTime(s, -1000))} className="py-2 rounded bg-gray-700 hover:bg-gray-600 text-gray-300 text-sm transition">-1秒</button>
            <button onClick={() => onUpdate((s) => adjustTime(s, 1000))} className="py-2 rounded bg-gray-700 hover:bg-gray-600 text-gray-300 text-sm transition">+1秒</button>
            <button onClick={() => onUpdate((s) => adjustTime(s, 10000))} className="py-2 rounded bg-gray-700 hover:bg-gray-600 text-gray-300 text-sm transition">+10秒 [→]</button>
          </>
        )}
      ...
      {p?.newaza_enabled && p.newaza_limit_type === "limited" && canAdjust && (
        <div className="flex gap-2 items-center justify-center mt-2">
          ...
          <button onClick={() => onUpdate((s) => adjustNewazaCount(s, 1))} className="px-3 py-2 rounded bg-gray-700 hover:bg-gray-600 text-gray-300 text-sm transition">-1</button>
...
function ExtensionButton({ props: { state, phase, p, onUpdate } }: { props: MatchOperationsProps }) {
  ...
  return <section><button onClick={() => onUpdate(startExtension)} className="w-full py-3 rounded-lg bg-purple-700 hover:bg-purple-600 text-white font-bold text-lg transition">延長戦へ</button></section>;
}
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-2 gap-2">
        <button onClick={() => { if (confirm("赤の棄権勝ちにしますか？")) onUpdate((s) => finishManual(s, "white", "withdraw")); }} className="py-2 rounded bg-gray-800 hover:bg-gray-700 text-gray-400 text-xs transition">赤棄権 → 白勝利</button>
        <button onClick={() => { if (confirm("白の棄権勝ちにしますか？")) onUpdate((s) => finishManual(s, "red", "withdraw")); }} className="py-2 rounded bg-gray-800 hover:bg-gray-700 text-gray-400 text-xs transition">白棄権 → 赤勝利</button>
      </div>
    </section>
  );
}
...
function UndoButton({ state, onUpdate }: { state: TimerState; onUpdate: (fn: (s: TimerState) => TimerState) => void }) {
  ...
  return <section><button onClick={() => onUpdate(undo)} className="w-full py-2 rounded bg-gray-800 hover:bg-gray-700 text-orange-400 text-sm font-bold transition">取消 [Esc] — {state.undoStack[state.undoStack.length - 1].action}</button></section>;
}

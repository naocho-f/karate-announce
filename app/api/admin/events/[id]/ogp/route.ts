...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: ev } = await supabaseAdmin.from("events").select("ogp_image_path").eq("id", id).single();
  if (ev?.ogp_image_path) {
    await supabaseAdmin.storage.from("form-notice-images").remove([ev.ogp_image_path]);
  }

  const ext = file.name.split(".").pop() || "jpg";
  const storagePath = `event-ogp/${id}/${Date.now()}.${ext}`;
  ...
  await supabaseAdmin.from("events").update({ ogp_image_path: storagePath }).eq("id", id);
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await supabaseAdmin.from("events").update({ ogp_image_path: null }).eq("id", id);

  ...
  return (
    <main className="min-h-screen bg-white text-gray-900 flex items-center justify-center p-6">
      <div className="max-w-sm w-full text-center space-y-4">
        ...
        <p className="text-gray-600">参加受付は終了しました。</p>
      </div>
    </main>
  );
}
  ...
  return (
    <main className="min-h-screen bg-white text-gray-900 flex items-center justify-center p-6">
      <div className="max-w-sm w-full text-center space-y-4">
        ...
        {isFetchError ? (
          <>
            <p className="text-gray-600">フォーム情報の取得に失敗しました。</p>
            <button onClick={() => window.location.reload()} className="text-blue-700 underline text-sm">
              再読み込み
            </button>
          </>
        ) : (
          <>
            <p className="text-gray-600">参加申込フォームは準備中です。</p>
  ...
  return (
    <main className="min-h-screen bg-white text-gray-900 flex items-center justify-center p-6">
      <div className="max-w-sm w-full text-center space-y-4">
        ...
        <p className="text-gray-600 text-sm">{displayName} さんの参加申込を受け付けました。</p>
        {emailSent && (
          <p className="text-gray-600 text-xs mt-2">確認メールを送信しました。届かない場合は迷惑メールフォルダをご確認ください。</p>
        )}
        <p className="text-gray-500 text-xs">{event.name}</p>
        <button onClick={onReset} className="text-blue-700 hover:text-blue-700 text-sm underline">
          別の方も申し込む
        </button>
      </div>
    </main>

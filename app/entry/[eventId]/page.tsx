...
function useEntryFormState(eventId: string) {
  ...
  const resetForm = useCallback(() => {
    setSubmitted(false); setEmailSent(false); setValues({}); setMultiValues({});
    setOtherValues({}); setConsents({}); setSelectedRules(new Set());
    setEmailConfirm(""); setError("");
  }, []);
    ...
    saveTimerRef.current = setTimeout(() => {
      try {
        sessionStorage.setItem(DRAFT_KEY, JSON.stringify({
          values, multiValues: Object.fromEntries(Object.entries(multiValues).map(([k, v]) => [k, [...v]])),
          otherValues, consents, selectedRules: [...selectedRules], emailConfirm,
        }));
      } catch { /* ignore */ }
    }, 500);
  ...
  return {
    values, setValues, multiValues, otherValues, setOtherValues, consents, selectedRules, setSelectedRules,
    submitting, setSubmitting, submitted, setSubmitted, emailSent, setEmailSent, error, setError,
    emailConfirm, setEmailConfirm, fieldErrors, setFieldErrors, DRAFT_KEY,
    setValue, setMultiValue, handleConsent, resetForm,
  };
}
...
function useEntryPageData(eventId: string) {
  ...
  useEffect(() => {
    fetch(`/api/public/form-config?event_id=${eventId}`)
      ...
      .then((data: FormConfigResponse) => { setFormConfig(data); setFormLoading(false); })
      .catch(() => { setFormConfig({ ready: false, fetchError: true } as FormConfigResponse); setFormLoading(false); });
  }, [eventId]);

  useEffect(() => {
    supabase.from("dojos").select("name, name_reading").order("name").then(({ data }) => { if (data) setDojoMaster(data); });
  }, []);
...
function isCheckboxFilled(key: string, values: Record<string, string>, multiValues: Record<string, Set<string>>,
  otherValues: Record<string, string>, visibleFields: Array<{ config: FormFieldConfig; def: FieldPoolItem }>): boolean {
...
function checkFieldFilled(
  key: string, def: FieldPoolItem, config: FormFieldConfig,
  values: Record<string, string>, multiValues: Record<string, Set<string>>,
  otherValues: Record<string, string>, visibleFields: Array<{ config: FormFieldConfig; def: FieldPoolItem }>,
): boolean {
...
export default function EntryPage({ params }: Props) {
  ...
  const { values, setValues, multiValues, otherValues, consents, selectedRules, setSelectedRules,
    submitting, setSubmitting, submitted, emailSent, setEmailSent, error, setError,
    emailConfirm, fieldErrors, setFieldErrors, DRAFT_KEY, setSubmitted,
    setValue, setMultiValue, handleConsent, resetForm, setOtherValues, setEmailConfirm,
  } = form;
  ...
  const canSubmit = useMemo(() => {
    ...
    for (const { config, def } of visibleFields) { if (!isFieldFilled(config, def)) return false; }
  ...
  const doSubmit = useCallback(async () => {
    ...
    const errors = validateEntry({ visibleFields, isFieldFilled, emailMismatch, emailConfirm, ageConflict, values, consents, notices });
  ...
  if (!data.formConfig?.ready) return <NotReadyScreen event={data.event} isFetchError={(data.formConfig as Record<string, unknown>)?.fetchError === true} />;
  if (submitted) return <SubmittedScreen event={data.event} displayName={[values["family_name"], values["given_name"]].filter(Boolean).join(" ") || "参加者"} emailSent={emailSent} onReset={resetForm} />;

  return (
    <EntryFormView event={data.event} isClosed={!!isClosed} notices={notices} fieldNotices={fieldNotices}
      visibleFields={visibleFields} formConfig={data.formConfig} values={values} multiValues={multiValues}
      otherValues={otherValues} fieldErrors={fieldErrors} emailConfirm={emailConfirm} emailMismatch={emailMismatch}
      ageConflict={ageConflict} eventRules={data.eventRules} ageCategories={data.ageCategories}
      dojoMaster={data.dojoMaster} consents={consents} submitting={submitting} canSubmit={canSubmit}
      error={error} hasRuleField={hasRuleField} selectedRules={selectedRules}
      onSetFieldErrors={setFieldErrors} onSetValue={setValue} onSetMultiValue={setMultiValue}
      onSetOtherValues={setOtherValues} onSetEmailConfirm={setEmailConfirm} onConsent={handleConsent}
      onSetSelectedRules={setSelectedRules} onSubmit={doSubmit} />
  );
}
...
function useAgeConflict(values: Record<string, string>, event: Event | null | undefined): string | null {
  return useMemo(() => {
    ...
    const hasBd = refDate.getMonth() > birth.getMonth() ||
...
function EntryFormView({ event, isClosed, notices, fieldNotices, visibleFields, formConfig, values, multiValues,
  otherValues, fieldErrors, emailConfirm, emailMismatch, ageConflict, eventRules, ageCategories, dojoMaster,
  consents, submitting, canSubmit, error, hasRuleField, selectedRules,
  onSetFieldErrors, onSetValue, onSetMultiValue, onSetOtherValues, onSetEmailConfirm, onConsent, onSetSelectedRules, onSubmit,
}: {
  event: Event; isClosed: boolean; notices: NoticeWithImages[]; fieldNotices: Record<string, NoticeWithImages[]>;
  visibleFields: Array<{ config: FormFieldConfig; def: FieldPoolItem }>; formConfig: FormConfigResponse;
  ...
  eventRules: { id: string; name: string }[]; ageCategories?: AgeCategory[];
  dojoMaster: { name: string; name_reading: string | null }[]; consents: Record<string, boolean>;
  submitting: boolean; canSubmit: boolean; error: string; hasRuleField: boolean; selectedRules: Set<string>;
  onSetFieldErrors: (e: Record<string, string>) => void; onSetValue: (key: string, val: string) => void;
  onSetMultiValue: (key: string, val: Set<string>) => void;
  onSetOtherValues: React.Dispatch<React.SetStateAction<Record<string, string>>>;
  onSetEmailConfirm: (v: string) => void; onConsent: (id: string, checked: boolean) => void;
  onSetSelectedRules: React.Dispatch<React.SetStateAction<Set<string>>>; onSubmit: () => Promise<void>;
}) {
  ...
  return (
    <main className="min-h-screen bg-main-bg text-white p-6">
      ...
      <div className="max-w-md mx-auto">
        ...
        <form onSubmit={(ev) => { ev.preventDefault(); void onSubmit(); }} className="space-y-6">
          ...
          <FieldRenderer visibleFields={visibleFields} formConfig={formConfig} values={values}
            multiValues={multiValues} otherValues={otherValues} fieldErrors={fieldErrors}
            emailConfirm={emailConfirm} emailMismatch={emailMismatch} ageConflict={ageConflict}
            event={event} eventRules={eventRules} ageCategories={ageCategories}
            dojoMaster={dojoMaster} fieldNotices={fieldNotices} consents={consents} inp={inp}
            onSetValue={onSetValue} onSetMultiValue={onSetMultiValue} onSetOtherValues={onSetOtherValues}
            onSetEmailConfirm={onSetEmailConfirm} onConsent={onConsent} />
          <FallbackRuleSelector hasRuleField={hasRuleField} eventRules={eventRules}
            selectedRules={selectedRules} onToggle={(id) => {
              onSetSelectedRules((prev) => { const next = new Set(prev); next.has(id) ? next.delete(id) : next.add(id); return next; });
            }} />
      ...
      <div className="max-w-md mx-auto">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0 space-y-1">
            ...
            {Object.values(fieldErrors).map((msg, i) => <p key={i} className="text-xs text-red-300/80">・{msg}</p>)}
          </div>
          <button onClick={onClear} className="text-red-300 hover:text-white text-lg leading-none shrink-0 mt-0.5" aria-label="閉じる">×</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <>
      {event.banner_image_path && (
        <Image src={`${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/form-notice-images/${event.banner_image_path}`}
          alt={event.name} className="w-full rounded-xl mb-4" width={800} height={400} unoptimized />
      )}
      ...
      {event.entry_close_at && !isClosed ? (
        <p className="text-xs text-yellow-400 mb-5">受付期限: {new Date(event.entry_close_at).toLocaleString("ja-JP", {
...
function FallbackRuleSelector({ hasRuleField, eventRules, selectedRules, onToggle }: {
  hasRuleField: boolean; eventRules: { id: string; name: string }[];
  selectedRules: Set<string>; onToggle: (id: string) => void;
}) {

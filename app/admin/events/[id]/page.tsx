...
type MatchRow = { tournament_id: string; fighter1_id: string | null; fighter2_id: string | null; round: number; rules: string | null };
...
type EventPageState = {
  event: Event | null; entries: Entry[]; entryRuleIds: Record<string, Set<string>>;
  eventRuleIds: Set<string>; tournaments: Tournament[]; rules: Rule[];
  mismatchSettings: MismatchSettings; tournamentMatchFighterIds: Record<string, Set<string>>;
  ...
  timerPresets: TimerPreset[]; step: 1 | 2 | 3;
  entrySubTab: "entries" | "form" | "email"; showClosedGuide: boolean;
  showAutoDialog: boolean; bracketRuleCount: number; bracketSubTab: "courts" | "bracket-rules";
  togglingClosed: boolean; entryCloseAtLocal: string; savingCloseAt: boolean;
  uploadingBanner: boolean; uploadingOgp: boolean; deletingImageType: "banner" | "ogp" | null;
  processingEntryIds: Set<string>; processingRuleKeys: Set<string>;
  currentFormVersion: number | null; formConfigVersion: number; ageCategories: AgeCategory[] | undefined;
};
...
type EventPageSetters = {
  ...
  setAllMatchRows: React.Dispatch<React.SetStateAction<Array<{ tournament_id: string; fighter1_id: string | null; fighter2_id: string | null }>>>;
...
function useEventPageState(): { state: EventPageState; setters: EventPageSetters } {
  ...
  const state: EventPageState = {
    event, entries, entryRuleIds, eventRuleIds, tournaments, rules, mismatchSettings,
    tournamentMatchFighterIds, savedMatchPairs, allMatchRows, timerPresets, step,
    entrySubTab, showClosedGuide, showAutoDialog, bracketRuleCount, bracketSubTab,
    togglingClosed, entryCloseAtLocal, savingCloseAt, uploadingBanner, uploadingOgp,
    deletingImageType, processingEntryIds, processingRuleKeys, currentFormVersion,
    formConfigVersion, ageCategories,
  };
  const setters: EventPageSetters = {
    ...
    setStep, setEntrySubTab, setShowClosedGuide, setShowAutoDialog, setBracketRuleCount,
    setBracketSubTab, setTogglingClosed, setEntryCloseAtLocal, setSavingCloseAt,
    setUploadingBanner, setUploadingOgp, setDeletingImageType, setProcessingEntryIds,
    setProcessingRuleKeys, setCurrentFormVersion, setFormConfigVersion, setAgeCategories,
  };
...
export default function EventDetailPage({ params }: Props) {
  ...
  const actions = useEventActions(id, {
    ...
    setEvent: setters.setEvent, setSavingCloseAt: setters.setSavingCloseAt,
    setEntryCloseAtLocal: setters.setEntryCloseAtLocal, setUploadingBanner: setters.setUploadingBanner,
    setUploadingOgp: setters.setUploadingOgp, setDeletingImageType: setters.setDeletingImageType,
    setProcessingRuleKeys: setters.setProcessingRuleKeys, setEntryRuleIds: setters.setEntryRuleIds,
    setProcessingEntryIds: setters.setProcessingEntryIds, setEntries: setters.setEntries,
  });
  const navigateStep = useCallback((s: 1 | 2 | 3) => {
    setters.setStep(s);
  ...
  const entryChangeSummary = useMemo(() => computeEntryChangeSummary(st.entries, st.tournaments, hasEntryChanges), [hasEntryChanges, st.entries, st.tournaments]);
  const allEntriesAssigned = useMemo(() => computeAllEntriesAssigned(st.entries, st.tournaments, st.tournamentMatchFighterIds), [st.entries, st.tournaments, st.tournamentMatchFighterIds]);
  ...
  return (
    <EventPageContent
      id={id} event={st.event} entries={st.entries} entryRuleIds={st.entryRuleIds}
      eventRules={eventRules} tournaments={st.tournaments} rules={st.rules}
      mismatchSettings={st.mismatchSettings} tournamentMatchFighterIds={st.tournamentMatchFighterIds}
      savedMatchPairs={st.savedMatchPairs} allMatchRows={st.allMatchRows} timerPresets={st.timerPresets}
      step={st.step} entrySubTab={st.entrySubTab} showClosedGuide={st.showClosedGuide}
      showAutoDialog={st.showAutoDialog} bracketRuleCount={st.bracketRuleCount}
      bracketSubTab={st.bracketSubTab} togglingClosed={st.togglingClosed}
      entryCloseAtLocal={st.entryCloseAtLocal} savingCloseAt={st.savingCloseAt}
      uploadingBanner={st.uploadingBanner} uploadingOgp={st.uploadingOgp}
      deletingImageType={st.deletingImageType} processingEntryIds={st.processingEntryIds}
      processingRuleKeys={st.processingRuleKeys} currentFormVersion={st.currentFormVersion}
      formConfigVersion={st.formConfigVersion} ageCategories={st.ageCategories}
      hasEntryChanges={hasEntryChanges} entryChangeSummary={entryChangeSummary} allEntriesAssigned={allEntriesAssigned}
      onNavigateStep={navigateStep} onSetEntrySubTab={setters.setEntrySubTab}
      onSetFormConfigVersion={setters.setFormConfigVersion} onSetBracketSubTab={setters.setBracketSubTab}
      onSetShowAutoDialog={setters.setShowAutoDialog} onSetEntryCloseAtLocal={setters.setEntryCloseAtLocal}
      onSetEvent={setters.setEvent}
      onToggleEntryClosed={() => void actions.toggleEntryClosed()} onSaveEntryCloseAt={() => void actions.saveEntryCloseAt()}
...
function computeAllEntriesAssigned(
  entries: Entry[], tournaments: Tournament[], fidsMap: Record<string, Set<string>>,
): boolean {
  if (tournaments.length === 0) return false;
...
function useEventActions(id: string, deps: EventActionDeps) {
  ...
  const toggleEntryRule = useCallback(async (entryId: string, ruleId: string) => {
    ...
    const res = await fetch("/api/admin/entry-rules", { method: has ? "DELETE" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ entry_id: entryId, rule_id: ruleId }) });
    if (res.ok) {
...
async function handleAutoCreateFromDialog(autoGroups: AutoGroup[], eventId: string, evtRules: Rule[], reload: () => void) {
  for (const group of autoGroups) {
    ...
    await fetch("/api/admin/tournaments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ courtName: group.name, courtNum: String(courtNum), pairs, eventId, type: "tournament", maxWeightDiff: group.maxWeightDiff, maxHeightDiff: group.maxHeightDiff }) });
  }
  reload();
}
...
function EventPageContent(props: {
  id: string; event: Event; entries: Entry[]; entryRuleIds: Record<string, Set<string>>;
  eventRules: Rule[]; tournaments: Tournament[]; rules: Rule[];
  mismatchSettings: MismatchSettings; tournamentMatchFighterIds: Record<string, Set<string>>;
  savedMatchPairs: Array<{ f1: string; f2: string; rules: string | null }>;
  allMatchRows: Array<{ tournament_id: string; fighter1_id: string | null; fighter2_id: string | null }>;
  timerPresets: TimerPreset[]; step: 1 | 2 | 3;
  entrySubTab: "entries" | "form" | "email"; showClosedGuide: boolean;
  showAutoDialog: boolean; bracketRuleCount: number;
  bracketSubTab: "courts" | "bracket-rules";
  togglingClosed: boolean; entryCloseAtLocal: string; savingCloseAt: boolean;
  uploadingBanner: boolean; uploadingOgp: boolean;
  deletingImageType: "banner" | "ogp" | null;
  processingEntryIds: Set<string>; processingRuleKeys: Set<string>;
  currentFormVersion: number | null; formConfigVersion: number;
  ageCategories: AgeCategory[] | undefined;
  hasEntryChanges: boolean; entryChangeSummary: string; allEntriesAssigned: boolean;
  ...
  onToggleEntryClosed: () => void; onSaveEntryCloseAt: () => void;
  ...
  return (
    <main className="min-h-screen bg-main-bg text-white p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <EventMetaSection event={p.event} eventId={p.id} onEventUpdate={(updates) => p.onSetEvent((prev) => (prev ? { ...prev, ...updates } : prev))} />
        <StepNav step={p.step} tournaments={p.tournaments} onStepChange={p.onNavigateStep} phaseStep={phase.stepHighlight} />
        {p.step === 1 && (
          <ParticipantSection
            eventId={p.id} event={p.event} entries={p.entries} entryRuleIds={p.entryRuleIds}
            eventRules={p.eventRules} processingEntryIds={p.processingEntryIds}
            processingRuleKeys={p.processingRuleKeys} currentFormVersion={p.currentFormVersion}
            formConfigVersion={p.formConfigVersion} ageCategories={p.ageCategories}
            entrySubTab={p.entrySubTab} showClosedGuide={p.showClosedGuide}
            entryCloseAtLocal={p.entryCloseAtLocal} savingCloseAt={p.savingCloseAt}
            togglingClosed={p.togglingClosed} uploadingBanner={p.uploadingBanner}
            uploadingOgp={p.uploadingOgp} deletingImageType={p.deletingImageType}
            onSetEntrySubTab={p.onSetEntrySubTab} onSetFormConfigVersion={p.onSetFormConfigVersion}
            onToggleEntryClosed={p.onToggleEntryClosed} onSaveEntryCloseAt={p.onSaveEntryCloseAt}
            onClearEntryCloseAt={p.onClearEntryCloseAt} onSetEntryCloseAtLocal={p.onSetEntryCloseAtLocal}
            onUploadEventImage={p.onUploadEventImage} onDeleteEventImage={p.onDeleteEventImage}
            onToggleRule={p.onToggleRule} onToggleWithdrawn={p.onToggleWithdrawn}
            onDeleteEntry={p.onDeleteEntry} onLoad={p.onLoad}
            onNavigateStep={p.onNavigateStep} onSetEvent={p.onSetEvent}
          />
        )}
        {p.step === 2 && (
          <BracketSection
            eventId={p.id} event={p.event} entries={p.entries} entryRuleIds={p.entryRuleIds}
            eventRules={p.eventRules} tournaments={p.tournaments}
            tournamentMatchFighterIds={p.tournamentMatchFighterIds} rules={p.rules}
            mismatchSettings={p.mismatchSettings} savedMatchPairs={p.savedMatchPairs}
            bracketRuleCount={p.bracketRuleCount} allMatchRows={p.allMatchRows}
            timerPresets={p.timerPresets} ageCategories={p.ageCategories}
            bracketSubTab={p.bracketSubTab} hasEntryChanges={p.hasEntryChanges}
            entryChangeSummary={p.entryChangeSummary} allEntriesAssigned={p.allEntriesAssigned}
            showAutoDialog={p.showAutoDialog} onSetBracketSubTab={p.onSetBracketSubTab}
            onSetShowAutoDialog={p.onSetShowAutoDialog} onNavigateStep={p.onNavigateStep}
            onLoad={p.onLoad} onHandleAutoCreateFromDialog={(...args) => void handleAutoCreateFromDialog(...args)}
          />
        )}

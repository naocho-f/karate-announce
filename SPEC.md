...
- **試合開始ボタン（▶ 試合開始）**: `ready` 状態かつ両選手揃っている試合のフッターに表示。クリックで AI アナウンス実行＋状態を `ongoing` に変更
...
- **試合番号バッジ**: ブラケットカードの**左上オーバーレイ**として `match_label` を表示（試合中は黄色、次の試合は青、終了は灰色半透明）。フッターのスペースを占有しない
- **次の試合ナビゲーションバナー**: トーナメントごとにブラケット上部に2行構成のバナーを表示
  - 上段: 試合番号・状態 ＋ 操作ヒント（右寄せ）
  - 下段: 選手名 vs 選手名（長い場合は truncate）
  - 試合中（`ongoing`）: 黄色バナー「第X試合 試合中 / 勝者をタップして確定」
  - 次の試合あり（`ready`）: 青バナー「次の試合：第X試合 / カードをタップして開始」。該当カードはパルスアニメーション付き青ボーダーで強調
  - 全試合終了: 緑バナー「全試合終了」

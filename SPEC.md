...
- **スコア行レイアウト**: 横幅均等3分割。左=赤スコア+反則インジケータ / 中央=寝技タイマー / 右=白スコア+反則インジケータ
- **スコア表示モード（プリセット設定に応じて切替）**:
  ...
  - ポイント＋技あり（`show_points=true, show_wazaari=true`）: 上部2/3にポイント、下部1/3に「技」ラベル＋技あり数を表示
- **反則インジケータ**: 各スコアエリアの外側（赤は左端、白は右端）に「反則」ラベル＋縦4セル（①下〜④上）を配置。反則数以下のセルが選手色で塗りつぶし（0回=全暗、1回=①のみ、2回=①②、3回=①②③、4回=全塗り）
- **合わせ一本オーバーレイ**: `combined_ippon` 判定時にスコア行全体（3分割の親コンテナ）に `position: relative` を設定し、その中に `absolute inset-0` で `bg-black/80` オーバーレイを配置。テキスト「{勝者名}選手の合わせ一本勝ち」（勝者名は `state.red.name` / `state.white.name` から取得、名前なしの場合は「合わせ一本」）をレスポンシブフォントサイズ（`min(10vw,4rem)`）で白文字表示。2秒後にフェードアウト
...
- **寝技ボタン残回数表示**: `newaza_limit_type === "limited"` の場合、寝技ボタンの下に「残りN回」を表示（`newaza_max_count - usedCount`）。残り0回でボタンが無効化される
...
- **試合一覧（カード形式）**: コート画面の速報と同様のカード形式。各カードに選手名・所属・ルール・トーナメント名を表示。`match_label` の数値順のみでソート（ステータスグループ順ではない）。ステータスに応じた表示:
  - ongoing: 黄色枠＋「試合中」バッジ（パルスアニメーション）
  - ready（最初の1件のみ）: 青枠＋「次の試合」バッジ。2件目以降の ready は通常表示
  - waiting: 暗い背景、選手名は「未定」表示、クリック不可（`cursor-not-allowed`）
  - done: グレーアウト＋「終了」バッジ（選択不可、`cursor-not-allowed`）
- **試合一覧に戻る**: ready/running/paused 状態で「← 試合一覧に戻る」ボタンを表示
- **次の試合へ**: finished 状態で結果確定済み（`resultWritten === true`）の場合のみ「次の試合へ」ボタンを表示。ボタンを押すと idle（試合一覧）に戻り、次の試合（最初の `ready`）の1つ前の試合位置に自動スクロール（`scrollIntoView({ block: "start" })`）。前の試合がない場合はリスト先頭にスクロール。結果未確定時はボタン自体を非表示にすることで誤操作を防止
...
- **レイアウトエディタ**: インライン表示（モーダル廃止）。行ベースのビジュアルエディタ。行の追加（目立つ破線ボタン）・削除・並べ替え（D&D、⠿ハンドル）、フォントサイズ（vh数値、上限なし）・高さ・配置を自由設定。▶/▼で展開/折りたたみ。表示ラベル設定は常時表示（アコーディオン廃止）。区切り線の太さは1px固定（UIから非表示）。スコア間隔・項目間隔もUIから非表示（内部デフォルト値を使用）

...
> 最終更新: 2026-03-26（自由設問バッジ拡張・複製/削除ボタンのデザイン改善・def取得バグ修正）
...
- 自由設問バッジの拡張: FIELD_POOL 由来でも大会ごとに聞くか変わる項目（保護者名、試合経験、希望試合数、頭突き希望、防具関連）に紫の「自由設問」バッジを表示。削除・複製ボタンは管理者追加の `custom_*` フィールドのみ。大会運営に必須の固定項目（氏名、性別、年齢、身長、体重、所属、ルール、連絡先等）はバッジなし・削除不可

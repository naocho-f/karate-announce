...
- **2行レイアウト**: 各試合行は1行目に試合番号＋ステータス、2行目に選手名 vs 選手名で改行なく表示
...
- **所属団体**: プレビューに「設定 > 道場/団体マスター」へのリンクと説明を表示。未登録団体は「その他」選択で自由入力
...
- **「＋ トーナメントを追加」ボタン**（グループ追加）: 未割当エントリーが0名になったら非表示
- **「＋ コートにトーナメントを追加する」ボタン**（確定後の新規作成）: 未割当エントリーが0名になったら非表示
- **確定後**: BracketView（トーナメントブラケット表示）
...
- 開閉なし・常時展開表示
- **コート別グループ表示**: ステップ②と同じ順・同じグループ分けで表示（コート見出し → その下にトーナメント一覧）。コートにトーナメントがなければそのコートは表示しない

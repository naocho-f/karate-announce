...
- **フォームプレビュー型UI**: 実際のエントリーフォームに近い見た目で項目を表示し、その場で設定変更が可能
  - カードは**ヘッダー部**（操作バー: ラベル・必須/任意タグ・▲▼移動・詳細・トグル）と**プレビュー部**（実際のフォーム入力の見た目）に分離
  - ホバーでツールバーが表示され、▲▼移動・必須/任意切り替え・「詳細」テキストボタンが操作可能
  - **トグルスイッチ**: 各項目右端にトグルで表示/非表示を切り替え。非表示時はプレビュー部に「このフィールドはフォームに表示されません」と表示

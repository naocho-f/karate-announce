...
> 最終更新: 2026-03-28（タイマー操作パネル改善13件: 試合一覧ハイライト・confirm削除・ルールラベル・戻るボタン・フルハイトレイアウト・勝利方法ボタン化・確定フロー改善・均等配置・反則設定表示・ボタン拡大・ストップ表記・全角数字・フルスクリーン）
...
- **試合一覧**: ready 状態の試合を青枠でハイライト、done の試合はグレーアウト＋「終了」バッジ表示（選択不可）
- **試合一覧に戻る**: ready/running/paused 状態で「← 試合一覧に戻る」ボタンを表示
- **次の試合へ**: finished 状態で「次の試合へ」ボタンを押すと idle（試合一覧）に戻る
...
- **反則ポイント設定表示**: 試合中にプリセットの反則→ポイント変換設定（`foul_to_point_start`）の有効/無効を常時表示
...
__tests__/
  ...
  e2e/            # E2E テスト（Playwright）
    ...
    timer-control-improvements.spec.ts # タイマー操作パネル改善（ストップ表記・戻るボタン・一本confirm削除・勝利方法ボタン・確定フロー・h-screen・反則設定表示・フルスクリーン）
```

...
- ※ 組み合わせ提案（階級分けおすすめ）はおすすめ振り分けダイアログ（`SuggestCreateDialog`）に統合済み
...
**おすすめ振り分けダイアログ（`SuggestCreateDialog` / `components/suggest-create-dialog.tsx`）**
- 「おすすめ振り分けで作成」ボタン押下でモーダルダイアログ表示
...
- **おすすめ振り分けで作成**: 「💡 おすすめ振り分けで作成」ボタン。振り分けルールボタンの横に配置。クリックで `SuggestCreateDialog` を開く。旧DashboardCardの「階級分けおすすめ」機能を統合
...
- **組み合わせ提案アルゴリズム（階級分け）**: 体重をメインとし、年齢・性別・経歴・体格（身長）・段級で分割してバランス評価（◎△✕）。体重: 45/50/55/60/65/70/75/80kg、年齢: 15/18/20/25/30/31/35/40/45歳、身長: 155/160/165/170/175/180cm、経験: 3/5/7/10年、性別: 男女。2名未満グループの提案は除外。最大8件表示。おすすめ振り分けダイアログ（`SuggestCreateDialog`）で利用
...
- 対戦表作成UX改善（3件）:
  ...
  - おすすめ振り分けからワンクリック作成: `computeSuggestions()` を `lib/suggestions.ts` に抽出。`SuggestCreateDialog` コンポーネントでおすすめ条件を選択→プレビュー→対戦表作成が可能

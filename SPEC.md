...
- **参加受付切り替えボタン**: 3段階表示
  - フォーム設定未公開時: 「準備中」（灰色バッジ）。ボタンは「📋 準備中（フォーム設定を公開してください）」表示で disabled
  - `events.entry_closed = false` かつフォーム公開済: 受付中（緑バッジ・緑ボタン「🔓 受付中（クリックで締め切り）」）
  - `events.entry_closed = true`: 受付終了（赤バッジ「🔒 受付終了（クリックで再開）」）
  - 管理者が自由に切り替え可能
...
- **年代フィルタ**: 下限・上限レンジ（`minGrade`/`maxGrade`）で範囲指定。`gradeToNumber()` で数値変換してフィルタリング。トーナメント名に「小1〜小4」形式で反映
- **フィルタ並び順**: 年代 → 年齢 → 体重 → 身長 → 性別 → 経験 → 名前。トーナメント名への反映は年代・年齢・体重・身長・性別のみ（経験・名前は除外）

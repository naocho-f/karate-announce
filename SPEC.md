...
> 最終更新: 2026-03-28（年代区分フィールド改善: grade をセレクト化、年齢ベース区分の設定UI追加）
...
- `branch`: 道場・支部名 + 読み仮名
- `grade`（年代区分）: セレクトボックスで表示。固定選択肢（年少/年中/年長/小1〜小6/中1〜中3）+ 年齢ベース区分（デフォルト: 18歳未満/一般/シニア）。年齢ベース区分は設定タブの「年代区分」で編集可能（`settings` テーブルの `age_categories` キー）。DBカラム `grade` に保存
...
- **参加者一覧**（`EntriesSection`）
  ...
  - カウント表示: 「N名参加 / M名欠場」形式
  - **テストデータ生成**（開発環境のみ表示）: 「🧪 テスト参加者を追加」ボタンで、参加申込フォームの全項目（性別・生年月日・読み仮名・電話番号・メールアドレス・都道府県・保護者名・試合経験・防具情報等の extra_fields を含む）が入ったダミー参加者32名を一括生成。`is_test = true` で識別。「🗑 テスト参加者を削除」ボタンで一括削除可能。各ボタンにはホバー時にツールチップで動作説明を表示
...
- ※ 組み合わせ提案（階級分けおすすめ）は参加者分布パネル（`DistributionPanel`）で表示（対戦表作成機能なし、参考情報として分布を表示）
...
- 振り分けルールが0件の場合は「振り分けルールを登録して対戦表を作成」ボタンを表示し、押下で振り分けルールサブタブに遷移
...
**参加者分布パネル（`DistributionPanel`）**
- CourtSection 内の「振り分けルールで対戦表を作成」ボタンの上に表示。未割当選手がいる場合のみ表示
...
- コート数に応じてコートタブを表示
- **振り分けルールで対戦表作成**: 「振り分けルールで対戦表を作成（N名）」ボタン（振り分けルール登録済みの場合）または「振り分けルールを登録して対戦表を作成」ボタン（未登録の場合）。未割当選手がいる場合のみ表示。クリックで `AutoCreateDialog` を開くか振り分けルールタブに遷移
- **参加者分布パネル**: 「💡 参加者の分布（N名）」ボタン。振り分けルールボタンの上に折りたたみ式で配置。体重・年齢・性別・身長・経験の分布をピル形式で表示（表示のみ、対戦表作成アクションなし）
...
- **選手絞り込みフィルタ**（トーナメントのみ）: 体重範囲、年齢範囲、性別、身長範囲、年代区分（セレクト）、経験、名前で選手を絞り込んでトーナメントに追加
...
- **組み合わせ提案アルゴリズム（階級分け）**: 体重をメインとし、年齢・性別・経歴・体格（身長）・段級で分割してバランス評価（◎△✕）。体重: 45/50/55/60/65/70/75/80kg、年齢: 15/18/20/25/30/31/35/40/45歳、身長: 155/160/165/170/175/180cm、経験: 3/5/7/10年、性別: 男女。2名未満グループの提案は除外。最大8件表示。参加者分布パネル（`DistributionPanel`）で利用
- **フィルター拡張**: 身長範囲・経験・年代区分フィルタをトーナメント作成UIに追加。フィルター条件はDBに永続化（`filter_experience`, `filter_grade`, `filter_min_height`, `filter_max_height`）
...
- デモエントリー年齢分布改善: 小学生30%・中高生25%・成人25%・中高年20%の年齢分布。年齢に応じた体重・身長レンジ。年代区分（幼稚園〜中学・18歳未満・一般・シニア）を自動設定
...
__tests__/
  unit/           # 単体テスト（Vitest）
    ...
    grade-options.test.ts    # 年代区分選択肢生成・gradeToNumber変換ロジック
  ...
  e2e/            # E2E テスト（Playwright）
    ...
    tournament-creation.spec.ts    # 対戦表作成（参加者追加・振り分けルール・ワンマッチ・削除）
...
- 単体テスト: 367 テスト（15 ファイル）
- API ルートテスト: 203 テスト（10 ファイル）
- E2E テスト: 47 テスト（10 ファイル）
- **合計: 570 単体/API + 47 E2E テスト**

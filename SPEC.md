...
> 最終更新: 2026-03-25（コート名の英字・日本語間スペース自動挿入、古いAndroid端末対策）
...
- **進行中試合の強調表示**: 試合中の行は太い青枠（`border-2 border-blue-400`）＋青いグロー（`shadow`）で強調。一目で現在進行中の試合を識別可能
...
- **選手絞り込みフィルタ**: 体重範囲、年齢範囲、性別、学年、名前で選手を絞り込んでトーナメントに追加

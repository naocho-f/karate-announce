...
> 最終更新: 2026-03-31（E2Eテスト安全性修正: 既存データ保護ルール追加）
...
- **スコア行レイアウト**: 横幅均等3分割。左=赤ポイント+反則インジケータ / 中央=寝技タイマー / 右=白ポイント+反則インジケータ
- **反則インジケータ**: 各ポイントエリアの外側（赤は左端、白は右端）に縦4セル（①下〜④上）を配置。反則数以下のセルが選手色で塗りつぶし（0回=全暗、1回=①のみ、2回=①②、3回=①②③、4回=全塗り）。技あり数・反則数のテキスト表示は廃止

...
> 最終更新: 2026-03-25（出場希望ルール単一/複数選択切替、所属団体その他チェック、エントリーボタンテキスト修正）
...
- **DB管理フィールド**（所属団体・出場希望ルール）: 選択肢はDB管理のため「選択肢設定」ボタンを非表示。代わりに設定画面へのリンクと説明を表示
  - 所属団体: 「設定 > 道場/団体マスター」へのリンク。未登録団体は「その他」で自由入力
  - 出場希望ルール: 「設定 > ルール管理」で登録済みのルール名をチェックボックスで表示。対戦表作成時のルール振り分けとの連携を説明。ヘッダーから複数選択/単一選択を切り替え可能（`custom_choices` に `__single_select__` マーカーで管理）

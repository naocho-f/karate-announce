...
- **試合番号バッジ**: ブラケットカードのフッター左端に `match_label` を表示（試合中は黄色、次の試合は青、終了は灰色）
- **次の試合ナビゲーションバナー**: トーナメントごとにブラケット上部に状態バナーを表示
  - 試合中（`ongoing`）: 黄色バナー、選手名表示、「勝者をタップして確定」
  - 次の試合あり（`ready`）: 青バナー、次の選手名表示、「カードをタップして開始」。次の試合カードはパルスアニメーション付き青ボーダーで強調
  - 全試合終了: 緑バナー「全試合終了」

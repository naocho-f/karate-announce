...
- **交流会テンプレート専用レイアウト（2026-04-13）**: %指定グリッドによる専用レンダラーを実装。上部50%(タイマー65%+寝技35%)・下部50%(選手名15%+スコア85%)の固定構造。各領域のフォントサイズをエディタで個別調整可能（12項目のスライダー+数値入力）。反則・技有のラベルは25%/セルは残り75%均等割

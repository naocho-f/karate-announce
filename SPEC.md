...
- **勝利オーバーレイ**: 試合終了（`isFinished`）かつ引き分け以外の場合、スコア行全体に勝者色の半透明オーバーレイ（`winnerColor + B3`、約70%透過）を常時表示（フェードアウトなし）。テキストは `resultDisplayText(state)` をレスポンシブフォントサイズ（`min(10vw,4rem)`）で白文字中央表示。一本・合わせ一本・ポイント等すべての勝利方法で統一表示

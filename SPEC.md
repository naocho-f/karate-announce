...
- **フォームプレビュー型UI**: 実際のエントリーフォームに近い見た目で項目を表示し、その場で設定変更が可能
  - 各項目はラベル＋入力プレビュー（テキスト入力、ラジオ、チェックボックス、セレクト等）を表示
  - ホバーでツールバーが表示され、▲▼移動・必須/任意切り替え・⚙詳細設定・✕非表示が可能
  - 読み仮名フィールドは親フィールドに従属表示（親が非表示なら読み仮名も非表示）
- **非表示項目リスト**: フォーム下部に非表示項目一覧を表示、「フォームに追加」ボタンでON
- **詳細設定（⚙展開）**: 選択肢のラベル編集・「その他」オプション追加

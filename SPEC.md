...
- **合わせ一本オーバーレイ**: `combined_ippon` 判定時に勝者サイド色でフラッシュ表示。一本オーバーレイと同様のエフェクト（2秒フェードアウト）。テキスト「合わせ一本」をレスポンシブフォントサイズ（`min(16vw,6rem)`）で表示
...
- **次の試合へ**: finished 状態で結果確定済み（`resultWritten === true`）の場合のみ「次の試合へ」ボタンを表示。ボタンを押すと idle（試合一覧）に戻る。結果未確定時はボタン自体を非表示にすることで誤操作を防止

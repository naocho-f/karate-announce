...
> 最終更新: 2026-03-29（対戦表作成UX改善・不具合報告に「修正中」ステータス追加）
...
- **組み合わせ提案（`computeSuggestions` / `lib/suggestions.ts`）**: 年齢・体重のしきい値で分割したとき、各グループの人数バランスを評価して提案を生成
...
- **おすすめ振り分けで作成**: 「おすすめ振り分けで作成」ボタン。振り分けルールボタンの横に配置。クリックで `SuggestCreateDialog` を開く

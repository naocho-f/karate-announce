...
> 最終更新: 2026-04-30（SW 登録ガード化 + Sentry bot UA フィルタ追加）
...
- **SW 登録ガード化 + Sentry bot UA フィルタ（2026-04-30）**: `SwRegister` を `useSyncExternalStore` で書き換え、`document.readyState === "complete"` かつ `document.prerendering` 偽の状態でのみ `SerwistProvider` をレンダリングするよう変更。bot/crawler や prerender 状態での `InvalidStateError: Failed to register a ServiceWorker` 発生を予防。`instrumentation-client.ts` の `Sentry.init` に `beforeSend` を追加し、`Chrome/X.0.0.0` 偽装 UA からのイベントと SW 登録失敗イベントを除外
...
- **Service Worker登録コード追加（2026-04-13）**: SerwistProviderコンポーネントでSW登録。本番環境でオフラインモードが実際に動作するようになった
- **交流会レイアウトのサイズ指定をコンテナ相対に変更（2026-04-13）**: フォントサイズをvhからコンテナ高さ比率のpx計算に変更。プレビューと実際のタイマー画面が完全一致、モニターサイズに依存しない
- **注意セル点灯時の文字色を黒に変更（2026-04-13）**: 交流会レイアウトの注意セルが点灯（黄色背景）時、文字色をグレーから黒に変更し視認性向上
- **タイマー試合番号をA-1形式に変換表示（2026-04-13）**: 交流会レイアウトの試合番号を「A第1試合」→「A-1」形式に変換。matchLabelToShort関数をmatch-utilsに追加
- **一時停止中のタイマー文字色をストップボタンの色に統一（2026-04-14）**: paused時のメインタイマー文字色をストップボタンの背景色と同じ色に変更。running時は既存のtimer_colorのまま
- **pre-commit警告をブロックに変更（2026-04-14）**: Phase 2の新規警告検出時にコミットを失敗させるよう変更。sw-register警告をベースラインに追加
- **試合開始ブザー機能追加（2026-04-14）**: プリセットに開始ブザー設定（auto/manual/off + 音源/秒数/回数）を追加。開始ボタン押下時に自動鳴動可能
- **アナウンス再生中オーバーレイ（2026-04-14）**: 再生中に全画面半透明オーバーレイを表示しキーボード・ボタン操作を無効化。再生停止ボタンで即時中断可能。stopSpeech関数追加
- **Step 4レビュー強化（2026-04-14）**: テスト削除時カバレッジ対応表必須化、lib/ export未使用検出手順追加、UI確認を本番サイトに変更
...
- **アナウンステンプレート取得修正（2026-04-14）**: 認証不要の公開API `/api/public/announce-settings` 新設。非admin画面からテンプレート取得不可だったバグ修正。エラー黙殺(.catch(() => {}))を廃止。テンプレート変数にコート名・トーナメント名を追加。デフォルトテンプレートをエラー検知用に変更
- **アナウンス画面リンク変更（2026-04-14）**: 試合タブのアナウンス画面リンクを別タブではなく同一タブで開くように変更
- **SW defaultCache廃止（2026-04-14）**: JSチャンクのSWキャッシュがデプロイ後も古いコードを配信する問題を修正。defaultCacheを削除し古いキャッシュを明示削除
- **match_labelからコートプレフィックス除去（2026-04-14）**: 自動採番で「B第4試合」のようにコート名が含まれていた問題を修正。「第4試合」に変更。既存DBデータも修正済み
- **試合ラベルのひらがな変換廃止（2026-04-14）**: 「第N試合」等のひらがな変換をやめ漢字のままTTSに渡す。ひらがな連結でTTSが誤読する問題を修正。固定読み（決勝・準決勝等）は維持
- **ルール必須化（2026-04-14）**: トーナメント作成・更新時にdefaultRuleName必須バリデーション追加。UIで登録ボタンをルール未選択時に無効化。トーナメント3のdefault_rulesを交流大会Bに修正
- **タイマー操作パネル刷新（2026-04-14）**: キーボードショートカット全廃止。右側パネルをショートカット一覧から操作履歴パネルに変更（幅拡大）。取消ボタンを目立つデザインに変更、アニメーション付きundo
- **match_numberカラム追加・タイマー試合番号表示改善（2026-04-14）**: matchesテーブルにmatch_number(integer)追加。試合番号保存時にmatch_numberも同時保存。タイマー表示をコート名頭文字+match_number形式（例: B-5）に変更。クイック試合ボタン削除。ショートカットキー表記の残り([G],[B],[←],[→])を削除
- **テスト参加者の物理削除対応（2026-04-14）**: テスト参加者(is_test=true)の削除を論理削除から即時物理削除に変更。DELETE APIに?hard=trueパラメータ追加、API側でis_test=trueを必ず確認しテスト参加者以外の物理削除を拒否
- **論理削除UIの視認性改善（2026-04-14）**: 削除予定アイテムのopacity-20を本体コンテンツ部分のみに適用し、DeletePendingBar（削除取消・今すぐ消す・削除予定日時）は通常の視認性を確保。参加者テーブル行はopacity-50に変更
- **スコア操作パネルのショートカットキー表記削除（2026-04-14）**: +1pt/技あり/反則/注意/一本ボタンからキー表記([Q],[W],[E],[D],[R],[I],[O],[P],[K],[L])を削除。keysプロパティも除去
- **操作パネル下部の重複取消ボタン削除（2026-04-14）**: \_match-operations.tsxのUndoButtonを削除。操作履歴パネル側の取消ボタン（アニメーション付き）に統一
- **対戦表の「第N試合の勝者」表示をmatch_numberに修正（2026-04-14）**: pendingSlotLabelがposition連番を使っていたのをmatch_number参照に変更。試合番号設定後は正しい番号で表示
- **累積モード寝技タイマー表示修正（2026-04-15）**: 寝技解除後にカウントダウンが設定値（2分等）に戻るバグを修正。寝1表示の条件がactive時のみnewazaDispMsを使っていたのを、累積モードでinactive時もelapsedMs>0ならnewazaDispMsを表示するよう修正
- **寝技タイマー各回の経過時間を個別記録（2026-04-15）**: NewazaStateにrounds配列を追加。寝技解除・タイムアップ時に各回の区間経過時間を記録。寝1・寝2の表示をroundsベースに変更し、解除後も正確な経過/残り時間を表示
- **メインタイマーのコロン位置修正（2026-04-15）**: TimerDigitsのコロンに不要なposition:relative+bottom調整がかかっていたのを削除。コロンが数字の中央に表示されるよう修正
- **交流大会テンプレートのデフォルトフォントサイズ調整（2026-04-15）**: DEFAULT_KOURYUUKAI_FONT_SIZESの各フォントサイズ値を実際の表示に合わせて調整。Prettierフォーマット整形も同時適用
- **交流会テンプレート寝技無制限時の表示簡略化（2026-04-15）**: 寝技回数無制限時、寝技表示を上段のみ（高さ100%）に変更し、ラベルの番号部分を非表示にして「寝」のみ中央寄せ表示
- **寝技rounds記録を消費確定時のみに修正（2026-04-15）**: 無消費解除（freeRelease以内）時にrounds配列に記録されていたバグを修正。消費確定した回のみroundsに追加するよう変更
- **累積モード寝技の無消費解除ロールバック・表示累積計算修正（2026-04-15）**: 無消費解除時にelapsedMsを今回区間分差し引いて「なかったこと」にする。寝技N回目の表示をrounds[0]〜[N]の累積加算で計算するよう修正

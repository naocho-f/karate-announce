...
- **交流会レイアウト色修正（2026-04-13）**: 反則セル点灯色を赤、試合番号ラベルを黄色に修正。反則・技有セル文字色を常にグレー(#A0A0A0)に統一。寝技2はラベル表示+時間のみ開始後表示。プレビューをKouryuukaiLayout直接使用に変更（旧PvCell等のプレビュー専用コンポーネント削除）

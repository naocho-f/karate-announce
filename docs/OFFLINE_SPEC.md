...
| Phase 4 | 4-4/4-6: **実施済み**、他: 不要 | swap_with トランザクション化 + 端末事前準備チェックリスト |
...
| ファイル | 操作 | 内容 |
|---------|------|------|
| ~~`components/queue-status-badge.tsx`~~ | — | `unified-status-bar.tsx` に統合済み |
...
> **ステータス**: 4-4/4-6 実施済み（コミット 15〜16）。他は不要と判断。
...
**受け入れ基準**:
- [x] swap_with が position を正しく交換すること（既存テスト通過）
- [x] 3ステップの逐次更新がなくなること
...
**受け入れ基準**:
- [x] SW 未登録時に「準備が必要です」が表示される
- [x] SW 登録済み + データキャッシュ構築済みで「準備完了」が表示される
- [x] データキャッシュ未構築時に「各画面を一度開いてください」と表示される
...
全フェーズ（Phase 1〜3 + Phase S + Phase 4）が実施済み。
...
【実施済み】Phase 4（コミット15〜16）
  → swap_with トランザクション化 + 端末事前準備チェックリスト
```

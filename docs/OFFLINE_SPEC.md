...
> 最終更新: 2026-04-07
> ステータス: **Phase 1+S 完了。Phase 2 実装予定**
...
| Phase 2 | **確定実施** | 操作キュー（IndexedDB）+ データキャッシュ + オフラインモード対応 |
| Phase 3 | **確定実施** | 控えめな楽観的更新 + TTS プリキャッシュ |
...
| 問題 | 影響 | 対応フェーズ |
|------|------|:----------:|
| リトライ全失敗で操作が消失する | 長時間オフラインで操作ロスト | Phase 2 |
| オフラインモードがない | 意図的オフライン運用ができない | Phase 2 |
| ポーリング失敗時のデータフォールバックなし | オフライン時にデータ表示不可 | Phase 2 |
| 操作の即時フィードバックが弱い | 勝者設定後に待たされる | Phase 3 |
| TTS 音声がオンライン必須 | オフラインでアナウンス不可 | Phase 3 |
...
**受け入れ基準**:
- [ ] オフラインモード切替 → 青色情報バー表示、警告バナー非表示
- [ ] オフラインモード中は操作が全てキューに保存される（リトライなし）
- [ ] ブラウザリロード後もオフラインモードが維持される
- [ ] オフラインモード中に接続テスト成功 → 「オンラインに切り替えますか？」確認表示
- [ ] 手動切替トグルが全画面のヘッダーに表示される
...
**受け入れ基準**:
- [ ] オンラインモードでオフライン中に勝者設定 → 「送信待ち」表示 + バッジ「未送信: 1件」
- [ ] オフラインモードで勝者設定 → 「保存済み」表示 + 青バー「保存済み: 1件」
...
| # | 施策 | 内容 |
|---|------|------|
| ~~4-1~~ | ~~ポーリング間隔の動的調整~~ | 不要。観客1000人規模でもREST APIのレート制限に抵触しない |
...
| ~~4-7~~ | ~~ストレージ容量管理~~ | 不要。TTS キャッシュは prefetchTts 呼び出し時に前回分を削除する設計に変更済み |
...
Phase 1+S が実施済み。大会が間近でも **瞬断・不安定接続には既に対応済み**。
Phase 2（オフラインモード + キュー）と Phase 3（楽観的更新 + TTS キャッシュ）は引き続き実装する。
...
Phase 2（2a〜2d）— 見積もり: 2.5〜3 週間
  ├ コミット9: 操作キュー基盤 + データキャッシュ
  ├ コミット10: キュー再送 + タブ間排他
  ├ コミット11: オフラインモード + 統合ステータスバー
  └ コミット12: 各画面へのキュー統合

  到達状態: 「完全オフラインでも運用できるアプリ」
  - 操作が永続保存され、復帰時に自動送信
  - オンライン/オフラインモードの切替が可能
  - オフラインモードでは警告なし、「保存済み: N件」の情報表示
  - オフラインでもキャッシュデータを表示

Phase 3（3a〜3b）— 見積もり: 1.5〜2 週間
  ├ コミット13: 控えめな楽観的更新
  └ コミット14: TTS プリキャッシュ
...
| IndexedDB の暗号化 | 管理端末は共有端末でない前提。Phase 4 でログアウト時のクリーンアップを検討 |

...
| entry_closed           | boolean     | true              | 手動受付終了フラグ（デフォルト true=受付終了。false に切り替えると受付開始し、`form_configs.is_ready` が自動で `true` に設定される） |
...
| データ | コピー | 備考 |
|--------|--------|------|
| イベント基本情報 | ○ | name, event_date は新規値。court_count, court_names, max_weight_diff, max_height_diff, banner_image_path, ogp_image_path, email_subject_template, email_body_template, venue_info, notification_emails, entry_close_at は元をコピー |
| ルール（event_rules） | ○ | |
...
| 振り分けルール（bracket_rules） | ○ | 大会設定の一部として常にコピー |
...
| トーナメント（tournaments） | entries コピー時のみ | matches も含めてコピー。status は `preparing` にリセット |
| 対戦者（fighters, event_fighters, event_fighter_rules） | entries コピー時のみ | entries の fighter_id を新 fighter に紐づけ |
| 試合（matches） | entries コピー時のみ | winner, done 等の結果はリセット |
| ステータス | リセット | status=`preparing`, is_active=`false`, entry_closed=`true` |
...
試合当日のオペレーター向け画面。アクティブな試合の操作に特化し、進行に迷わないフォローを提供する。

**データ取得**: 全イベント + エントリー数 + トーナメント有無。10秒タイムアウト付き。
...
- 「試合を管理する →」ボタン → 試合タブへ
- 「操作説明を見る」ボタン → 操作説明タブへ
- **次の試合**: `event_date` が未来で最も近いイベントのカウントダウン表示
- **要対応**: 参加者がいるがトーナメント未作成のイベントを警告表示

#### 状態B: アクティブな試合がある場合（メイン）

**ヘッダー**: 大会名 + 日付 + コート数 + 参加者数 + 受付状態

**コート操作カード** (2列グリッド):
各コートごとに3つのリンク:
| リンク | 遷移先 | 用途 |
|--------|--------|------|
| 🎤 コート進行 | `/court/{n}` (新タブ) | 試合の開始・判定操作 |
| ⏱ タイマー操作 | `/timer/{n}/control` (新タブ) | タイマーのキーボード操作 |
| 📺 表示画面 | `/timer/{n}` (新タブ) | 外部モニター用タイマー表示 |

**クイックリンク**:

- 📡 試合速報（観客用） → `/live` (新タブ)
- 速報URLをコピー
- 管理画面 → `/admin/events/{id}`

**操作ガイド** (アコーディオン、3セクション):

- **コート画面の使い方**: 試合開始 → 勝者判定 → 自動進行の流れ
- **タイマーの使い方**: キーボード操作、外部モニター表示
- **困ったときは**: 訂正、棄権、ミュート、音声設定、試合番号変更
...
1. 新規作成フォーム（トグル）
2. 進行中・予定の試合リスト
3. 過去・完了の試合リスト（折り畳み、デフォルト非表示）

**過去/完了の判定**:

- `status === "finished"` のイベント
- `event_date` が今日より前の日付のイベント
- 上記に該当するイベントは「過去・完了の試合（N件）」として折り畳み表示
...
- `status === "finished"` のイベントには「完了」バッジ

**アクション**:
| ボタン | 動作 | 表示条件 |
|--------|------|---------|
| ▶ アクティブに設定 / 進行中（クリックで停止） | `is_active` トグル | status !== "finished" |
| ✓ 完了にする | `status: "finished"`, `is_active: false` に更新 | status !== "finished" |
| 再開する | `status: "preparing"` に戻す | status === "finished" |
| 管理画面を開く → | `/admin/events/{id}` に遷移 | 常時 |
| アナウンス画面 | `/` を新しいタブで開く | active 時のみ |
| コピー | 複製モーダルを表示 | 常時 |
| 削除 | 確認ダイアログ → DELETE | 常時 |

**イベント作成フォーム**:

- 入力: イベント名、開催日（任意、過去日付は選択不可）、コート数（1〜4）、コート名、ルール
- 開催日の `min` 属性で今日以降のみ選択可能。API でも過去日付は 400 エラー
...
- **編集**: 読み仮名・説明をインライン編集
- **削除**: 確認ダイアログ付き
- **タイマー紐づけ**: 各ルールにタイマーを紐づけ可能。ルール一覧にタイマー名をバッジ表示。ドロップダウンで選択（値を保持）、✕で解除。タイマーが0件の場合はタイマータブへの誘導を表示。1件以上の場合はドロップダウン末尾に「＋ 新規追加」リンク（タイマータブへ遷移）
- **データ**: `rules` テーブルに保存。紐づけは `rules.timer_preset_id` で管理（各ルールに1つのタイマーを紐づけ）

#### タイマー管理

- 設定タブ内にインライン表示（モーダル廃止、フォームとプレビューを横並び）
- タイマーの CRUD + 複製 + レイアウトエディタ（行追加ボタンは破線で目立たせ、展開/折りたたみアイコン付き）+ プレビュー（sticky固定）
- 詳細は TIMER_SPEC.md 参照
...
サブタブではなく、1画面にフォーム設定・メール設定・参加受付を統合表示。フォーム設定とメール設定はアコーディオン（クリックで展開/折り畳み）。

#### フォーム設定カード

- ヘッダーにステータスバッジ（公開中: 緑、準備中: 黄、未設定: グレー）
- クリックで展開 → `FormConfigPanel` を表示
- 詳細は FORM_CONFIG_SPEC.md 参照

#### メール設定カード

- ヘッダーにステータスバッジ（設定済み: 緑、デフォルト: グレー）
- クリックで展開 → `EmailSettingsPanel` を表示
- **初期値**: 件名・本文にデフォルトテンプレート（`DEFAULT_SUBJECT` / `DEFAULT_BODY`）が入力済みの状態で開始
- 未カスタマイズ時もデフォルトテンプレートでメール送信される
...
- 2段階表示: 「受付中」（緑）↔「受付終了」（赤）。受付開始時に `is_ready` が自動で `true` になりフォームが公開される
...
- **テストデータ**（開発環境のみ）: 「🧪 テスト参加者を追加」ボタンでダミー32名生成、「🗑 テスト参加者を削除」で `is_test: true` のエントリーを一括削除。各ボタンにホバーツールチップで動作説明あり。テストデータ生成時はイベントのフォーム設定（form_field_configs）を参照し、選択肢・チェック項目はフォームに登録されている値のみを使用する
...
観客・保護者・選手が特定の選手の試合が近づいたら通知を受けられる機能。

**ウォッチ登録**:

- ヘッダーの「👁 ウォッチ」ボタンからウォッチパネルを開く
- テキスト入力で選手名を検索（全試合の fighter 名から部分一致フィルタ、重複除去）
- 候補をタップしてウォッチリストに追加（選手名単位。勝ち上がりの全試合を自動監視）
- ウォッチリストは `localStorage` に永続化（キー: `karate_watch_list`）

**通知判定**:

- `load()` 後に全試合を走査
- ウォッチ選手が出場する試合の `match_label` から番号を抽出（例:「Aコート第8試合」→ 8）
- 同コートで `(番号 - 3)` 以降の試合が `ongoing` になったらリマインド通知
- `match_label` 未設定の試合は通知対象外
- 通知済みの試合は `Set<matchId>`（useRef）で重複防止。リロード時にリセット

**通知UI**:

- 画面上部にバナー表示: 「○○選手の試合がもうすぐ開始します。Aコート赤までお越しください」
- 赤/白はその試合で fighter1（赤）か fighter2（白）かから判定
- タップまたは10秒で自動消去
- `navigator.vibrate([200, 100, 200])` で振動通知（対応端末のみ）
...
- [x] イベント複製: 参加者コピー時はトーナメント・対戦者・試合もコピー（結果はリセット）。バナー・テンプレート・振り分けルール等は常時コピー

...
- `is_test = true` のエントリ（テスト参加者追加ボタンで生成されたもの）は、削除時に論理削除ではなく即時物理削除する
- DELETE `/api/admin/entries/[id]?hard=true` で物理削除を実行。API側で `is_test = true` を必ず確認し、テスト参加者以外は物理削除を拒否する
- テスト参加者は本番データではないため、復元不要・即時完全削除が適切
...
- 本体コンテンツ部分のみ `opacity-20` で暗く表示
- DeletePendingBar（削除予定日時・削除取消・今すぐ消す）は `opacity-20` の外に配置し、通常の視認性を確保

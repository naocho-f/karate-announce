...
```json
{
  "config_id": "uuid",
  "fields": [
    /* FormFieldConfig[] — temp_ prefix ID は新規カスタムフィールド */
  ],
  "notices": {
    "upsert": [
      /* FormNotice[] — images/created_at 除外。temp_ prefix ID は新規 */
    ],
    "delete_ids": [
      /* 削除する notice の real ID */
    ]
  },
  "custom_fields": {
    "create": [
      /* { field_key, label, field_type, choices } — 新規 def 情報 */
    ],
    "delete_keys": [
      /* 削除する field_key */
    ]
...
- `FormConfigStatusBadge`: ステータス（公開中/準備中/未設定）+ バージョン表示を統合表示
  - version=0: 「準備中 未公開」（黄）
  - version>=1 かつ is_ready=true: 「公開中 v{version}」（緑）
  - version>=1 かつ is_ready=false: 「準備中 v{version}」（黄）
...
- 新規注意書き（未保存、temp\_ ID）の場合、画像アップロードは disabled（「保存後に画像を追加できます」ツールチップ）
...
- `deletedNoticeIds: string[]` — 削除した注意書きの real ID（temp\_ ID は追跡しない）
